...
- (void)_decodeImageFromElement:(DTHTMLElement *)element options:(NSDictionary *)options
{
	...
	if (_displaySize.width == 0 || _displaySize.height == 0)
	{
		...
		if (!_image)
		{
			image = [self imageForKey:[contentURL absoluteString]];
		}
		...
		if (!image)
		{
			...
			if (image)
			{
                [self saveImage:image forKey:[contentURL absoluteString]];
			}
		}
...
#pragma mark - Cache
- (DTImage *)imageForKey:(NSString *)key {
    
    return [[DTCache sharedInstance] objectForKey:key];
}

- (void)saveImage:(DTImage *)image forKey:(NSString *)key {
    
    [[DTCache sharedInstance] setObject:image forKey:key];
}
...
- (DTImage *)image
{
	if (!_image)
	{
		if (_contentURL)
		{
			DTImage *image = [self imageForKey:[_contentURL absoluteString]];
			...
			if (!image && [_contentURL isFileURL])
			{
				...
				if (image)
				{
                    [self saveImage:image forKey:[_contentURL absoluteString]];
				}
			}
