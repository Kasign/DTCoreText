...
- (void)didMoveToSuperview
{
    ...
    if (!self.image && (_url || _urlRequest) &&
        ...
        && self.superview)
    {
        UIImage *image = [self imageForKey:_url.absoluteString];
...
- (void)completeDownloadWithData:(NSData *)data
{
    ...
    if (_url)
    {
        ...
        {
            // cache image
            [self saveImage:image forKey:_url.absoluteString];
        }
        else
        {
...
#endif
{
    ...
    if (!&CGImageSourceCreateIncremental || !self.shouldShowProgressiveDownload)
    {
