...
- (void)_registerTagEndHandlers
{
    ...
    void(^tableBlock)(void) = ^{
        __strong typeof(weakSelf) strongSelf = weakSelf;
        DTTableTextAttachment *tableAttachment = strongSelf->_currentTag.textAttachment;
        if ([tableAttachment isKindOfClass:[DTTableTextAttachment class]]) {
            [tableAttachment didFinishParserElement];
        }
    };
    [_tagEndHandlers setObject:[tableBlock copy] forKey:@"table"];
}
...
//        DTLogDebug(@"--->>> Parser Start: %@", elementName);
...
//            DTLogDebug(@"--->>> Parser End: %@", elementName);
