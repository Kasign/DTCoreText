...
+ (instancetype)elementWithName:(NSString *)name attributes:(NSDictionary *)attributes options:(NSDictionary *)options
{
    ...
    Class class = [DTRegister registeredElementClassForTagName:name];
    
    if (!class)
    {
        // see if this is a custom attachment class
        Class attachmentClass = [DTRegister registeredAttachmentClassForTagName:name];
...
- (NSDictionary *)attributesForAttributedStringRepresentation
{
    ...
    // set underline style
    if (_underlineStyle && _underlineType <= 0)
    {
#if DTCORETEXT_SUPPORT_NS_ATTRIBUTES
        if (___useiOS6Attributes)
        {
    ...
    // 厚度
    if (_decorationThickNess > 0) {
        [tmpDict setObject:[NSNumber numberWithFloat:_decorationThickNess] forKey:DTDecorationThickNessAttribute];
    }
    
    if (_underlineType > 0) {
        [tmpDict setObject:[NSNumber numberWithFloat:_underlineType] forKey:DTUnderLineTypeAttribute];
        [tmpDict setObject:[NSNumber numberWithInteger:1] forKey:DTUnderLineAttribute]; // 后面作为开关使用
    }
...
- (void)applyStyleDictionary:(NSDictionary *)styles
{
    ...
    if (decorationThickNess && [decorationThickNess isKindOfClass:[NSString class]]) {
        self.decorationThickNess = [decorationThickNess pixelSizeOfCSSMeasureRelativeToCurrentTextSize:_currentTextSize textScale:_textScale];
    }
    ...
    [self extentStyles:styles];
}

// 扩展字段
- (void)extentStyles:(NSDictionary *)styles {
    
    NSString *underlineColor = [[styles objectForKey:@"underline-color"] lowercaseString];
    if (underlineColor && [underlineColor isKindOfClass:[NSString class]]) {
        self.underlineColor = DTColorCreateWithHTMLName(underlineColor);
        self.underlineType = 10;
        self.decorationThickNess = 6;
    }
    
    // 下划线类型
    NSString *underlineType = [[styles objectForKey:@"underline-type"] lowercaseString];
    if (underlineType && [underlineType isKindOfClass:[NSString class]]) {
        if ([underlineType isEqualToString:@"rectangle"]) {
            self.underlineType = 10;
            self.decorationThickNess = 6;
        } else if ([underlineType isEqualToString:@"wave"]) {
            self.underlineType = 20;
            self.decorationThickNess = 1;
        } else if ([underlineType isEqualToString:@"quadrangle"]) {
            self.underlineType = 30;
            self.decorationThickNess = 12;
        }
    }
    
    // 下划线高度
    NSString *decorationThickNess = [[styles objectForKey:@"underline-thickness"] lowercaseString];
    if (decorationThickNess && [decorationThickNess isKindOfClass:[NSString class]]) {
        self.decorationThickNess = [decorationThickNess pixelSizeOfCSSMeasureRelativeToCurrentTextSize:_currentTextSize textScale:_textScale];
    }
}
...
- (void)inheritAttributesFromElement:(DTHTMLElement *)element
{
    ...
    _underlineType = element.underlineType;
    _decorationThickNess = element.decorationThickNess;
