		...
		for (DTCoreTextGlyphRun *oneRun in oneLine.glyphRuns)
		{
			...
			if (runRange.location >= skipRunsBeforeLocation)
			{
				...
				if (linkURL && !attachment)
				{
					...
					subviewFrame = [oneLine frameOfGlyphsWithRange:effectiveRangeOfLink];
				...
				else
				{
					...
					if (attachment)
					{
						...
						subviewFrame = CGRectMake(oneRun.frame.origin.x, oneLine.baselineOrigin.y - ascender, oneRun.frame.size.width, ascender+descender);
					}
					else
					{
						subviewFrame = oneRun.frame;
					}
				}
				
				// if there is an attachment then we continue even with empty frame, might be a lazily loaded image
				if ((subviewFrame.size.width <= 0 || subviewFrame.size.height <= 0) && !attachment)
				{
					continue;
				}
				...
				{
					subviewFrame.origin.x += _layoutOffset.x;
					subviewFrame.origin.y += _layoutOffset.y;
				}
				
				// round frame
				subviewFrame.origin.x = floor(subviewFrame.origin.x);
				subviewFrame.origin.y = ceil(subviewFrame.origin.y);
				subviewFrame.size.width = round(subviewFrame.size.width);
				subviewFrame.size.height = round(subviewFrame.size.height);
				
				if (CGRectGetMinY(subviewFrame)> CGRectGetMaxY(rect) || CGRectGetMaxY(subviewFrame) < CGRectGetMinY(rect))
				{
				...
				if (attachment)
				{
					...
					if (existingAttachmentView)
					{
						...
						existingAttachmentView.frame = subviewFrame;
					...
					else
					{
						...
						if ([attachment isKindOfClass:[DTDictationPlaceholderTextAttachment class]])
						{
							...
							newCustomAttachmentView.frame = subviewFrame; // set fixed frame
						}
						else if (_delegateFlags.delegateSupportsCustomViewsForAttachments)
						{
							newCustomAttachmentView = [_delegate attributedTextContentView:self viewForAttachment:attachment frame:subviewFrame];
						}
                        if (!newCustomAttachmentView && _delegateFlags.delegateSupportsGenericCustomViews)
						{
							...
							newCustomAttachmentView = [_delegate attributedTextContentView:self viewForAttributedString:string frame:subviewFrame];
						}
				...
				if (linkURL && (_delegateFlags.delegateSupportsCustomViewsForLinks || _delegateFlags.delegateSupportsGenericCustomViews))
				{
					...
					if (subviewFrame.size.height < oneLine.frame.size.height)
					{
						subviewFrame.origin.y = trunc(oneLine.frame.origin.y);
						subviewFrame.size.height = ceil(oneLine.frame.size.height);
					}
					
					if (existingLinkView)
					{
						existingLinkView.frame = subviewFrame;
					...
					else
					{
						...
						if (_delegateFlags.delegateSupportsCustomViewsForLinks)
						{
							...
							newCustomLinkView = [_delegate attributedTextContentView:self viewForLink:linkURL identifier:guid attributes:attributes frame:subviewFrame];
						}
						
                        if (!newCustomLinkView && _delegateFlags.delegateSupportsGenericCustomViews)
                        {
                            NSMutableAttributedString *string = [layoutString attributedSubstringFromRange:runRange].mutableCopy;
                            [string addAttributes:oneRun.attributes range:NSMakeRange(0, string.length)];
                            newCustomLinkView = [_delegate attributedTextContentView:self viewForAttributedString:string frame:subviewFrame];
                        }
	...
	DTBlockPerformSyncIfOnMainThreadElseAsync(^{
		...
		if (strongSelf->_layoutFrame && strongSelf.superview)
		{
			...
			if (strongSelf->_attributedString)
			{
				...
				CGRect optimalFrame = CGRectMake(strongSelf.frame.origin.x, strongSelf.frame.origin.y, neededSize.width, neededSize.height);
...
- (void)setDelegate:(id<DTAttributedTextContentViewDelegate>)delegate
{
    ...
    _delegateFlags.delegateSupportsCustomViewsForLinks = [_delegate respondsToSelector:@selector(attributedTextContentView:viewForLink:identifier:attributes:frame:)];
